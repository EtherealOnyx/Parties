...
public class PEffectsD extends PEffects{
    ...
    public PEffectsD(String name, int x, int y, int width, int height) {
    ...
    void renderSelf(int i, ClientPlayerData id, ForgeIngameGui gui, PoseStack poseStack, float partialTicks) {
        if (id.effects.sizeBad() > 0) {
            start(poseStack, i, id.effects.sizeBene(), Config.rB(), Config.mB());
            AtomicInteger iX = new AtomicInteger();
            AtomicInteger iY = new AtomicInteger();
            if (id.effects.largerBad()) {
                id.effects.forEachBadLim((effect) -> {
                ...
                if (renderOverflow(gui, poseStack, i, iX.get(), iY.get(), partialTicks)) {
                    ...
                    id.effects.forBadRemainder((effect) -> lC.add(new ColorComponent(new TranslatableComponent(effect.getEffect().getDescriptionId()).append(" ").append(effect.getRoman()), effect.colorType())));
