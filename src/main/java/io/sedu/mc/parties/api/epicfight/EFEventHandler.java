...
public class EFEventHandler {
    ...
    public static void onPartyJoin(PartyJoinEvent event) {
        event.forTrackersAndSelf((sendTo, propOf) -> {
            InfoPacketHelper.sendStaminaUpdate(sendTo, propOf, PlayerData.playerList.get(propOf).getStamina());
            InfoPacketHelper.sendMaxStaminaUpdate(sendTo, propOf, PlayerData.playerList.get(propOf).getMaxStamina());
        });
    }
